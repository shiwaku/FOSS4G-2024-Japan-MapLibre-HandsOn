...
map.on("load", () => {
  ...
  map.addLayer({
    ...
    filter: ["!=", ["get", "buildingID"], "13103-bldg-27323"], // 指定されたIDの建築物を非表示
    // filter: [">=", ["get", "measuredHeight"], 100] // 高さが100m以上の建築物のみ表示
  });
});

// 建築物クリック時にポップアップを表示する
map.on('click', 'bldg-pmtiles', (e) => {
  var id = e.features[0].properties['id']; // idを取得
  var source = e.features[0].properties['source']; // sourceを取得
  // ポップアップを作成し、クリック位置に表示
  new maplibregl.Popup()
    .setLngLat(e.lngLat) // クリック位置にポップアップを設定
    .setHTML(
      // ポップアップの内容をHTML形式で設定
      '<div>id: ' + id + '</span></strong></div>' +
